...
Element::Element(SDL_Texture *_texture, SDL_Rect _rect, SDL_Renderer * renderer) :GuiObject()
{
	texture = _texture;
	m_renderer = renderer;
	rect = _rect;
...
void Element::Draw()
{
	if (Active) {
		...
		SDL_RenderCopyEx(m_renderer, texture, NULL, &rectangle, 0, NULL, flip); //SDL_RenderCopyEx(Window::Renderer(), texture, NULL, &rect, 0, NULL, flip);
